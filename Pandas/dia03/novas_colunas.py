...
# %%
# cria uma coluna e usa a função usando o apply
df["Name_First"] = df["Name"].apply( get_first )
df

# %%
# lambda é uma função anônima e simples muito usada quando quer executar uma função uma única vez em uma única linha de forma rápida
# seria uma prática melhor do que criar aquela função acima e deixa o código mais clean
df["Name"].apply( lambda x: x.upper().split("_")[0] )

# %%

# criando uma função para clusterizar os pontos e depois criando uma coluna para mostrar essa classificação
...
# criando uma função que vai receber uma linha e vou pegar o índice dela 
# usando o row para aplicar o apply nas colunas em formato de linha
